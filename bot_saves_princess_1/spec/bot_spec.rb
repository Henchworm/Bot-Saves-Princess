...
RSpec.describe Bot do
  ...
  context 'initialize' do
    ...
    describe '#path_to' do
      context 'when target is reachable in multiple steps' do
        let(:target) { Position.new(2, 0) }

        it 'returns the full shortest path as an array of moves' do
          expect(bot.path_to(target)).to eq(%w[DOWN LEFT])
        end

        it 'ends with the bot at the target position' do
          bot.path_to(target)
          expect(bot.position.same_position?(target)).to be true
        end
      end
    end
  end
